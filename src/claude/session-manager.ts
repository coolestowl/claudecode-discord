    ...
    try {
      ...
      const spawnViaSSH = project.workspace_name
        ? (({ command, args, cwd }: { command: string; args: string[]; cwd?: string; env: Record<string, string | undefined>; signal: AbortSignal }) => {
            ...
            const settingsIdx = claudeArgs.indexOf("--setting-sources");
            ...
            const remoteCmd = `cd ${singleQuote(cwd ?? config.CODER_REMOTE_HOME)} && ${envStr ? `env ${envStr} ` : ""}claude ${finalArgs.map(singleQuote).join(" ")}`;

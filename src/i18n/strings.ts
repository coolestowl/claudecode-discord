...
export const s_noAccessToken      = () =>
  L(
    "❌ `OAUTH_TOKEN` is not configured. Set it in your `.env` file to enable usage queries.",
    "❌ `OAUTH_TOKEN` 未配置，请在 `.env` 文件中设置以启用用量查询。",
    "❌ `OAUTH_TOKEN`이 설정되지 않았습니다. 사용량 조회를 활성화하려면 `.env` 파일에서 설정하세요.",
  );
